from news_data_scraper.document.document_classes import (
    NewsDocument,
    BusinessUnitSection,
    BusinessUnitHeader,
)
...
def create_document_contents(
    all_articles: NewsArticles, output_file_name: str, run_cga: bool
) -> NewsDocument:
    ...
    rn_news_section = BusinessUnitSection(
        header=BusinessUnitHeader(
            header="Retail & Networks", author="Fabian Schomerus", bu_tag=BuTag.ER_RN
        ),
    ...
    news_document = NewsDocument(
        business_unit_sections=[rn_news_section], name=output_file_name
    )

    if run_cga:
        cga_news_section = BusinessUnitSection(
            header=BusinessUnitHeader(
                header="Central Governments Advisory",
                author="Harry Allport",
                bu_tag=BuTag.ER_CGA,
            ),
